...
/* ── 지원자격 상용문구도 역량이 아니다 (실측 2026-09-15, 사용자 제보) ────────────
   '해외여행에 결격사유가 없는 자', '병역필 또는 면제자' 는 거의 모든 공고에 붙는
   자격 확인 문구지 직무가 요구하는 역량이 아니다. 그런데 '해외' 라는 낱말 때문에
   **글로벌·외국어 역량**이 뽑혔다(실측: 사람인 동원개발 공고).
   구간 필터(usefulText)는 [결격사유] 머리말이 있을 때만 걸리는데, 이 문구는 대개
   자격요건 안에 한 줄로 섞여 있어서 문장 단위로 막아야 한다. */
const DISQUALIFY = /결격\s?사유|병역(필|\s?면제|\s?기피)|해외여행에?\s*결격/;
