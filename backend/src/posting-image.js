...
/* ── 한 번에 보는 장수 (2 → 6, 실측 2026-09-15) ──────────────────────────────
   예전에는 2장이었다. 한 장에 최대 76초가 걸린 실측이 있어서(아래 OCR_TIMEOUT_MS
   주석) 석 장이면 상한(120초)을 넘길 수 있다고 봤기 때문이다.

   그런데 **세로로 긴 공고를 조각내어 보내기 시작하면서**(화면 prepareImage) 2장으로는
   모자라게 됐다. 조각은 통짜 한 장보다 작고 글자도 적어서 빠르다 — 1240×4496 짜리
   표를 넷으로 나눠 한 번에 보냈을 때 **16.9초** 였고, 항목 20개를 다 읽었다
   (같은 표를 552×2000 으로 줄여 한 장으로 보내면 18개였다).
   그래서 6장까지 연다. 통짜 큰 사진 여섯 장을 올리면 여전히 상한에 걸릴 수 있는데,
   그때는 실패 사유가 그대로 화면에 나가므로 조용히 틀리지 않는다. */
const MAX_IMAGES = Number(process.env.POSTING_OCR_MAX_IMAGES || 6);
