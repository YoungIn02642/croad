...
/* ── 포스터 (2026-09-07 꺼 둠 → 2026-09-14 사용자 결정으로 켠다) ──────────
   ── 무엇이 걸리는 일인가 ──
   목록의 사실(제목·주최·마감)을 인용하는 것과, **주최사의 저작물인 포스터를 받아
   우리 서버에서 다시 내보내는 것**은 무게가 다르다. 위비티 이용약관 제7조 2항도
   '제3자의 저작권을 침해하지 않아야 한다' 고 적고 있다. 그래서 처음에는 껐다 —
   위비티에 인용 허락을 문의해 두고 답을 기다리는 동안의 기본값이었다.

   ── 왜 켜는가 ──
   **사용자 결정이다(2026-09-14).** 답을 기다리는 사이 공모전 카드가 표지 없이
   비어 보인다는 지적이 반복됐고, "흐리게라도 넣어 달라" 는 지시로 완화책이 이미
   들어와 있다. 지금 걸려 있는 것은 이렇다:
     · 화면에서 `filter: blur(4px)` 로 흐리게 띄운다(hover 시 2px). 무슨 그림인지는
       알아보되 포스터 안의 글자는 읽히지 않는 선이다(mentoring.css .sup-cover-img).
     · 원본을 바로 가리키지 않는다. 화면은 `/api/specup/poster?id=` 만 알고,
       원본 주소는 서버가 캐시에서 찾는다 — 남의 대역폭을 쓰지 않고 핫링크도 아니다.
     · 카드를 누르면 원문으로 보낸다. 표지는 '무슨 공고인지 알아보게 하는' 용도다.
   **허락은 아직 받은 것이 아니다.** 답이 오거나 문제 제기가 들어오면
   `.env` 에 `WEVITY_POSTER=off` 한 줄로 즉시 되돌린다 — 그래서 스위치는 남긴다.

   꺼 두면 카드는 **주관기관 로고 → 이모지**로 물러난다. 화면 코드는 이미 그렇게
   되어 있어서 손댈 것이 없다(specup.js coverArt). */
const POSTER_ON = String(process.env.WEVITY_POSTER || 'on').toLowerCase() !== 'off';
