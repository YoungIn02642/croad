...
window.CompanyCover = (() => {
  ...
  let step     = 'overview';
  /* 최근 이슈의 두 갈래 — 'sim' 자리에 **주요 뉴스**(3개월 구간 대표), 'date' 자리에
     **최신 뉴스**(7일 이내). 한 자리에서 토글로 바꿔 보여준다(사용자 지시 2026-09-09,
     네이버 화면 캡처). id 는 예전 값을 그대로 둔다 — 이벤트·상태가 이 문자열을 쓰고
     있어서, 이름을 바꾸면 화면 말고 고칠 곳이 늘어난다.
     기본이 주요 뉴스인 것은 지원동기 소재로는 '이 회사에 무슨 일이 있었나' 가
     '오늘 뭐가 났나' 보다 먼저이기 때문이다(news.js ENDPOINTS 주석의 실측).
  ...
  function issueDetail(s) {
    ...
    /* ── 왼쪽 탭은 '주요 뉴스'(weekly)다 (사용자 지적 2026-09-14) ────────────────
       ── 무엇이 문제였나 ──
       "관련도순과 최신순이 별로 차이가 없다". 실측해 보니 그럴 수밖에 없었다 —
       관련도 목록(items)은 **회사명 검색의 sim 상위 5건**인데, 네이버 관련도 결과가
       최근에 몰려 있어서 날짜가 최신 목록과 같았다(삼성전자: items 5건이 전부 같은 날,
       현대자동차: 5건 중 3건이 최신 목록과 동일). 두 탭이 같은 며칠을 서로 다른
       순서로 보여준 셈이다.

       weekly 는 다르다. 3개월을 18일씩 끊어 구간마다 한 건씩이라 실측에서
       0일·27일·53일·55일·77일 전으로 **실제로 흩어진다.** 사용자가 이 탭에 기대한
       것도 그것이다("지금부터 1~2주 간격으로 주요 뉴스").

       ── 2026-09-09 에 반대로 바꿨던 자리다 ──
       그때 이유는 "탭에 '관련도순' 이라 적혔는데 시기별 대표를 보여주면 이름과 내용이
       어긋난다" 였다. 맞는 지적이라 **이번에는 이름 쪽을 고친다** — 탭을 '주요 뉴스 /
       최신 뉴스' 로 적으면 이름과 내용이 맞는다. 정렬 토글이 아니라 두 갈래 목록이다.

       weekly 가 비는 경우(웹 폴백 — 발행일이 없어 구간을 못 나눈다)에만 items 로
       내려간다. 그때는 날짜가 없으니 시기별이라고 말할 수 없고, 관련도 상위가 맞다. */
    ...
    return `
      <div class="co-news-sort">
        ${tab('sim', '주요 뉴스', sort === 'sim', false)}
        ${tab('date', '최신 뉴스', sort === 'date', !canDate)}
      </div>
