...
3. 팀으로 일하며 갈등을 해결한 경험을 기술해 주십시오.`;
  ...
  function applyPosting(r) {
    ...
    /* ── 무엇을 빼고 담았는지 말한다 (사용자 지시 2026-09-15) ────────────────────
       서버가 복리후생·결격사유·전형절차·근무조건을 빼고 담는다(server.js trimPosting).
       조용히 줄여 놓으면 "가져온 글이 왜 이것뿐이지" 가 되고, 잘못 잘렸을 때
       되돌릴 실마리도 없다 — 몇 자를 뺐는지 붙여 준다. 안 뺐으면 아무 말도 안 한다. */
    const cutMsg = r.trimmed
      ? ` 자소서에 쓰지 않는 부분(복리후생·전형절차·근무조건 등) ${r.trimmed.toLocaleString()}자는 빼고 담았어요.`
      : '';
    /* 이미지에서 읽은 글은 **더 의심하게** 말한다. OCR 은 본문 추출보다 깨지고,
       26-8 에서 "읽은 것을 그대로 분석에 넘기지 않는다" 고 미리 정해 뒀다. */
    if (r.fromImage) {
      const how = r.url ? '글이 없어 공고 이미지' : '올려 주신 이미지';
      urlMsg('warn', `${head}${how} ${r.fromImage}장을 읽었어요 (${size}자).${cutMsg} `
      ...
      /* 예전에는 '회사 소개·메뉴는 지우세요' 였다. 회사 소개는 이제 **일부러 남기므로**
         (지원동기 재료다) 지우라고 하면 안 된다 — 메뉴만 말한다. */
      urlMsg('ok', `${head}본문을 가져왔어요 (${size}자).${cutMsg} `
        + '메뉴·안내문이 섞여 있으면 지우고 분석해 주세요.');
    }
    ta.focus();
  }
