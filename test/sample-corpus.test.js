...
/* ── 4. 비슷한 문항 찾기 (B) ────────────────────────────────── */
{
  if (!CORPUS.isLoaded()) {
    ...
    /* ── 안 닮아도 유형이 잡히면 유형 중앙값이 나간다 (사용자 지시 2026-09-14) ──────
       예전에는 여기서 null 이었다. 그러면 협업처럼 **표본이 11편이나 있는데 문항끼리
       말이 달라 하나도 안 붙는 유형**이 통째로 빠진다. 근거의 세기가 다르므로
       블록이 스스로 어느 쪽인지 말해야 한다 — 그것까지 검사한다. */
    const loose = CORPUS.structureBlock('좋아하는 음식과 그 이유를 쓰시오', { typeId: 'trait', limit: 600 });
    ok('안 닮아도 유형 중앙값은 준다', !!loose);
    ok('유형 중앙값이라고 밝힌다', /같은 유형/.test(loose) && /느슨하게/.test(loose), '근거가 약한 값이라 말해야 한다');
    ok('비슷한 문항인 척하지 않는다', !/비슷한 문항\*\* \d+편/.test(loose));

    /* 유형을 모르면 분량표도 없어서 "분량표가 우선" 이 가리킬 곳이 없다. */
    ok('유형을 모르면 아무것도 주지 않는다',
      CORPUS.structureBlock('좋아하는 음식과 그 이유를 쓰시오', { typeId: null }) === null);
  }
}

/* ── 5. 검색이 문항 길이에 끌려다니지 않는다 (사용자 지시 2026-09-14) ──────────
   자카드는 분모가 합집합이라 **긴 문항일수록 점수가 깎였다**(표본 실측: 60자 넘는
   문항 적중 38%, 100자 넘으면 0%). idf 가중 포함률로 바꾼 자리를 고정한다. */
{
  const long = '당사 지원 직무와 관련하여 본인이 갖춘 경험이나 역량을 구체적인 사례를 들어 기술해 주시기 바랍니다. 해당 경험에서 본인의 역할과 결과를 반드시 포함해 주세요. (1000자 이내)';
  const short = '직무 관련 경험을 기술해 주세요.';
  ok('긴 문항이 짧은 문항을 담고 있으면 닮은 것으로 본다',
    CORPUS.similarity(long, short) >= CORPUS.MIN_SIM,
    `→ ${CORPUS.similarity(long, short).toFixed(2)}`);
  /* 흔한 말('경험을 기술해 주세요')만 겹치는 것은 닮은 것이 아니다 — idf 가 그걸 가린다. */
  ok('흔한 말만 겹치는 것은 안 닮은 것이다',
    CORPUS.similarity('좋아하는 음식을 기술해 주세요.', '직무 관련 경험을 기술해 주세요.') < CORPUS.MIN_SIM,
    `→ ${CORPUS.similarity('좋아하는 음식을 기술해 주세요.', '직무 관련 경험을 기술해 주세요.').toFixed(2)}`);

  /* '협업 경험'(2-gram 3개) 같은 표본은 아무 문항에나 0.5 넘게 붙는다. 검색 후보에서 뺀다. */
  const tiny = CORPUS.nearest('다른 사람들과 협업하여 공동의 목표를 달성한 경험을 기술해 주세요.', 'collab');
  ok('짧은 표본을 열쇠로 끌어오지 않는다',
    tiny.every(s => CORPUS.norm(s.question).replace(/\s/g, '').length >= CORPUS.MIN_GRAMS),
    `→ 잡힌 ${tiny.length}편`);

  /* 한 편만 잡히면 그 한 편에 끌려간다 — 협업 800자에서 '숫자 8개'(유형 중앙값은 4개)가 나왔다. */
  ok('한 편만 잡히면 비슷한 문항으로 치지 않는다', CORPUS.MIN_HITS >= 2);
}
